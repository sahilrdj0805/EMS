import { CalendarDays, FileText, Loader2, Send, X, Sparkles } from 'lucide-react';
...
const ApplyLeaveModal = ({open, onClose, onSuccess}) => {
    ...
    // Controlled inputs for AI generation
    ...
    const handleGenerateAI = async () => {
        ...
        try {
            ...
            // Auto-fill the textarea so the user doesn't even need to copy-paste!
            setReason(res.data.body);
            toast.success("AI draft generated and pasted successfully!");
        } catch (err) {
            toast.error("Failed to generate AI reason.");
        } finally {
            setGenerating(false);
        }
    }
        ...
        <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-lg animate-fade-in' onClick={(e)=>e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className='p-6 space-y-5'>
                 ...
                 <div>
                    ...
                    <select name="type" required value={leaveType} onChange={(e) => setLeaveType(e.target.value)}>
                 ...
                 <div>
                    ...
                    <div className='grid grid-cols-2 gap-4'>
                        <div>
                            ...
                            <input type="date" name="startDate" required min={minDate} value={startDate} onChange={(e) => setStartDate(e.target.value)} />
                        </div>
                        <div>
                            <span className="block text-xs text-slate-400 mb-1">To</span>
                            <input type="date" name="endDate" required min={minDate} value={endDate} onChange={(e) => setEndDate(e.target.value)} />
                        </div>
