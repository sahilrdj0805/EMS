...
import {CalendarIcon, ChevronRightIcon, DollarSignIcon, FileTextIcon, LayoutGridIcon, Loader2, LogOutIcon, MenuIcon, SettingsIcon, UserIcon, XIcon, SparklesIcon, BriefcaseIcon} from 'lucide-react'
...
const Sidebar = () => {
    ...
    const navItems = [
        ...
        role === "ADMIN" ?
        {name: "Departments", href: "/departments", icon: BriefcaseIcon} : null,
        ...
        <>
            ...
            <div className='flex-1 px-3 space-y-0.5 overflow-y-auto'>
                {loading ? (
                    ...
                    return (
                        <Link key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-emerald-500/15 text-emerald-300" : "text-slate-300 hover:text-white hover:bg-white/4"}`}>
  ...
  return (
    <>
        ...
        <aside className='hidden lg:flex flex-col h-full w-65 bg-linear-to-b from-teal-950 via-slate-900 to-slate-950 text-white shrink-0 border-r border-white/4'>
            {sidebarContent}
        </aside>
        <aside className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b from-teal-950 via-slate-900 to-slate-950 text-white z-50 flex flex-col transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
            {sidebarContent}
        </aside>
    </>
  )
